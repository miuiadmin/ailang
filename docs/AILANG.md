...
- **Part 0 · 前言与动机**（§1 摘要 / §1.5 Conformance 与规范性词汇（规范层）/ §2 为什么是 AILang / §3 四大核心原则 / §4 与现有语言的关系 / §5 语言导览 / §6 名称与宣言）
...
- **附录**（A 版本演进与收敛 / B 工程遗留 / C 结语 / D 术语表 Glossary）
...
## 1.5 Conformance、规范性词汇与行为分类

> 本节定义本规范的**规范性约束力**——何为「合规实现」、规范性词汇的含义、以及程序行为的分类法。本节是其余所有章节（§2–§94）的**元前提**：没有它，「§X 规定 Y」无法判断 Y 是强制约束还是示例说明。本节为 v0.2.1 治理地基补齐（P0-0）；版本号待 P0 路线完成时统一演进（见附录 A）。

### 1.5.1 规范性词汇（RFC 2119 中文映射）
...
| **应** | SHOULD / RECOMMENDED | 强烈建议；偏离须有正当理由并文档说明 |
| **可以** | MAY / OPTIONAL | 实现自由度；是否实现均合规 |

正文以中文词汇为准，附录 D 给出完整术语映射。仅当上述词汇以**粗体**形式出现、或所在条款明确声明其规范性时，才具 RFC 2119 约束力；普通行文中的「必须」「可以」不自动升级为规范性语义。

### 1.5.2 规范性内容与资料性内容（normative / informative）

- **规范性（normative）**：Part I 语言规范（§7–§27）、§24（`.ailmeta` schema）、本节 §1.5，以及 §83–§94 决议记录中标注 `[定义]`/`[语义]` 的条款——合规实现**必须**满足。
- **资料性（informative）**：Part 0（§1–§6 动机与导览）、Part III（§44–§64 教程示例）、Part IV（§65–§82 编译器实现设计）、附录 A/B/C，以及决议记录中标注 `[一致性]`/`[完整性]` 的说明性条款——仅供理解，**不**构成合规要求。
- 资料性内容与规范性内容冲突时，以规范性内容为准。教程示例（§45–§63）为资料性；当示例与 Part I 规则不一致时，以 Part I 为准。

### 1.5.3 Conformance：合规 AILang 实现

一个**合规的 AILang 实现**（conforming implementation）**必须**满足：

1. **接受并正确编译**所有符合 §27 形式文法、类型成立（§15）、所有权成立（§18/§74）的**安全代码**（safe code，即不含 `unsafe` 块的程序），并保证其行为符合本规范的规范性条款。
2. **遵守行为分类**（§1.5.4）：对安全代码，**不得**引入未定义行为。
3. **生成符合 §24 的 `.ailmeta`**（随包强制产物，§32），字段与排序遵循 §78。
4. **诊断**违反规范性条款的程序（类型错误、所有权违规、可见性违规等），诊断信息**应**结构化（§69.3）。

实现**可以**提供超出本规范的能力（扩展），但**不得**改变安全代码的规范性语义。`unsafe` 块（§25）与 FFI（`extern`，§25.3）是合规实现**唯一允许**偏离安全语义的逃生舱，其边界由 §90 #4 精确清单锁定。

### 1.5.4 程序行为分类（定义 / 未定义 / 实现定义 / 未指定）

本规范对程序行为作如下分类：

- **定义行为（defined behavior）**：本规范完整规定结果，合规实现**必须**产生该结果。安全代码的算术（§90）、析构顺序（§18.7 / §90 #5）、错误传播（§17）等均属此类。
- **未定义行为（undefined behavior, UB）**：本规范不施加任何约束——程序进入 UB 后，实现**可以**产生任意结果（含看似正常、崩溃或错误结果），且**不**保证可观测的失败。**AILang 的立场：安全代码（不含 `unsafe`/`extern`）永不产生 UB。** UB 仅可能来自：① `unsafe` 块内违反 §25.1 精确清单的操作；② 跨 `extern`（FFI）边界的不当交互（如 C 侧 `longjmp` 跨入，§90 #4）；③ 不安全代码触发的底层 UB（裸指针解引用无效、调用不匹配原型的 `extern` fn 等）。
- **实现定义行为（implementation-defined behavior）**：本规范要求实现**文档化**其选择，但允许实现间不同。v0.2.1 当前**未**登记任何实现定义行为（目标是最小化此类自由度）；未来若引入（如目标三元组相关的数据布局），**必须**在附录登记。
- **未指定行为（unspecified behavior）**：本规范允许若干合规结果之一，不要求实现文档化选择。当前主要未指定项：**表达式求值顺序**（实参、操作数、字面量元素的求值次序；除 `&&`/`||` 短路由语义内在规定外，未作规定）——此项已登记为待收紧（附录 B 遗留 #7），计划在后续 P0-2 收紧为左到右。

> 注：`panic`（§17，含 `unwrap`/`assert`、debug 整数溢出、除零、越界、约束违约）是**定义行为**（确定性栈展开 + 同序 Drop），**不是** UB。

### 1.5.5 Build profile（debug / release）

构建配置文件（build profile）控制编译期检查的强度与优化级别。合规实现**必须**支持至少两个 profile：

| profile | 语义 |
|---|---|
| **debug**（`ail build` / `ail run` 默认） | 开发期检测层**全开**：整数溢出 → panic `ArithmeticOverflow`（§90 #1）；契约 / 约束断言**必须**启用（§20 / §15.4）；边界检查启用（§90 #4）。 |
| **release**（`ail build --release`） | 优化构建：整数算术**二补数回绕**为规范语义（§90 #1），溢出 panic 关闭；契约 / 约束断言是否保留**可以**经配置控制（默认保留，可关闭以换性能）。 |

- profile 的具体配置项（`opt-level`、`overflow-checks`、`debug-assertions` 等）属工具链层，**应**在 `ail.toml` 的 `[profile.debug]` / `[profile.release]` 表声明（§30.1 待补）。
- **安全性不变量**：无论 profile，安全代码**不得**产生 UB（§1.5.4）。profile 仅调节开发期检测与优化级别，**不**改变安全代码的规范性结果（§90 #1 已锁定 release 回绕为规范语义，故溢出定性差异属规范许可、非 profile 引入的不一致）。
...
**上下文关键字**（仅特定语法位置保留、词法层产出 `Ident`、不入 56）：`where`（泛型 bound 子句，§15.10）、`constraint`（§15.4 声明引导词；与硬关键字 `meaning` 分类不同，为已知非对称）、`on`/`reply`/`detached`（§21 / §87）、`default`/`timeout`（select）、`reduce`（§87 #9）。另：`lock`/`yield`/`spawn_blocking` 为 `std.sync`/`std.async` 内建函数/控制构造（**非关键字**，§21 / §87 #7）；agent 块 `goal`/`tools`、actor 块 `state` 为**普通标识符字段标签**（非关键字、词法产出 `Ident`、约定名，§27）。
...
> **数值语义（§90 #1/#2/#3）**：整数算术溢出 = **定义行为**（永不 UB）——release **二补数回绕**（规范），debug **溢出 → panic `ArithmeticOverflow`**（开发期检测层）；整数**除零 / 取模零 → panic `DivideByZero`**（确定性）；浮点遵循 **IEEE 754**（`x / 0.0`（`x≠0`）→ `±inf`、`0.0 / 0.0` → `NaN`、NaN 经运算传播、`NaN == NaN` 为 `false`，**浮点运算永不 panic**）。显式运算符族（`wrapping_*`/`checked_*`/`saturating_*`/`overflowing_*`）/ 安全除法 `checked_div` 见 `std.math`（§33.1）；浮点 NaN 检测（`is_nan`/`is_inf`/`is_finite`）为 **float 方法** `x.is_nan()`（非自由函数，§94 #4）。
...
> **lang item**（§86 #5）：`Optional`/`Result`（及 `Ok`/`Some`/`None`、`List`/`Map`/`string` 等）为 **`#[lang]`** 标注的 std.core 类型——既是库类型，又被编译器特判（errors 派生 §17、`try/catch` 解糖 §83 #4、特权 `match` arm）。
...
**变体值构造（§89 #2）**：`Ok(x)` 变体构造**复用 call 文法**（`Variant(args)`），由名字解析消歧（变体 vs 函数）；裸变体 = `ident`；错误侧 = `E` 的命名变体（**无 `Err` 包装**，§16）。**成功返回须显式 `Ok(x)`，无 auto-wrap**（呼应 §11 禁隐式转换）；`Result<void, E>` 成功构造 = `return Ok(void)`（无 `()` 字面）。
...
| **COW 值类型** | `string` `List` `Map` `Set` | 值语义：赋值=共享缓冲（**原子引用计数**），修改=克隆（写时复制）；**为 `Send + Sync`**（§85 #2，原子 rc 共享只读线程安全）；行为同 Copy，无 `&str`/`String` 双类型 |
...
- **不逃逸**：禁止存入 struct 字段、禁止返回、禁止跨 `await` 存活 → 寿命 ⊆ 当前栈帧（由 Lifetime Analyzer 推导）→ **无需 `'a` 标注**。
...
- **`spawn` 作用域语义（§87 #4）**：三形式 `spawn <name>(args)`（具名 Task）/ `spawn actor Name(args)`（Actor，返 `ActorHandle`，§87 #5）/ `spawn { ... }`（匿名块）；具名/匿名返 `TaskHandle`。**默认 scoped**——task 附属 spawn 所在作用域，作用域末**隐式 join**（父等待子完成）；**显式分离用 `spawn detached`**（`detached` 为上下文关键字，不入 56）。**非** fire-and-forget（仍返回 `TaskHandle`、`cancel` 可达；§87 #4）。
- **`task` 与 `async fn` 区分**（§87 #1）：`task` = void body + CSP 通道（不可 async），经 Channel 投递消息 / `cancel` 通信、**不强制 `await`**；`async fn f() -> T` 的 `T` 即 `await` 产出值（调用得 `Future<T>` lang item，`await f()` 得 `T`）。**「须返回 Result」仅约束用户代码显式 `await` 的 async fn**；server route 等 runtime 派发 handler 豁免（返回声明类型即可）。**无 `Pin`**：borrow 禁跨 await（§18.5 / §74.2）→ 状态机不自引用。
- **并发错误传递**（§89 #8 统一表）：`throw` 仅合法于返回 `Result<T,E>` 的函数体（§17、§89 #6）——`async fn -> Result<T,E>` 可 `throw`/`try`（错误随 `await` 传）；`task`（void 体）/ actor `on` handler / server handler **禁 throw**，分别经 Channel 投递错误消息或 `cancel`、`reply(<错误变体>)`、runtime 派发传递。完整表见 §21.9。
...
- **Actor**（§87 #5）：`actor { state; message; on Msg { msg -> body } }`——`on` 为消息处理子句；`spawn actor Name(...)` 返回 `ActorHandle`；`h.send(Msg{...})` / `h ! Msg{...}` 投递；状态经 `self.state` 串行访问（无需锁）。**`reply`**（HIGH#5）：`on` 子句内的**上下文关键字**——`reply(expr)` 为该 handler 的值/错误出口（`reply` 值确定 message 的响应类型，可为 `Result<T, E>`）；actor `on` handler **禁 `throw`**，错误经 `reply(<错误变体>)` 传递（§89 #8）。
...
- 生命周期是否安全（被 spawn 的数据不能是 `borrow` 的非逃逸借用——`borrow` 不逃逸且禁跨 `await`，权威见 §18.5 / §74.2；本文 §21.3 释其与「无 Pin」的关系）
...
- **无 `Pin`**（相对 Rust async 的关键简化）：`borrow`/`borrow_mut` **不逃逸、禁跨 `await` 点**（权威：§18.5 / §74.2）→ async 状态机**永不自引用** → 无需 Rust 的 `Pin`。此「禁跨 `await`」是并发各节反复引用的拱顶规则（§21.2、§21.4 借用不逃逸均其推论）。
...
- `Channel` 为运行时支持的**共享 `Send` 句柄**（跨 spawn 克隆，底层队列）；`spawn { }` 块体为**控制构造**，可捕获外层不可变 / `Send` 变量（**不含 `borrow`**——借用不逃逸，详见 §21.3 / §18.5 / §74.2；非通用闭包，v0.2 无一等闭包）——见 §21、§84 #7。
...
`server` / `route` 为语言级构造，编译器生成路由 + 并发调度 + 元数据。handler 块为控制构造（§84 #7/#8），可捕获外层 `Send` 句柄（如连接池 `db`）。interface 值为 **Move 句柄**（§85 #6）：handler 捕获 owned 句柄，每次调用经形参模式推断 `borrow`（§84 #13），借用仅存于同步处理帧内、不跨 `await`（§18.5 / §74.2）。**handler 豁免 Result 规则**（§87 #1）：runtime 是唯一 awaiter，`async fn chat(req) -> Response` 返回声明类型即可（非用户 `await`）。
...
agent       := "agent" ident "{" (ident ":" string_lit)? (ident ":" "[" expr_list "]")? "}"   // 字段标签（约定 goal / tools）为普通标识符，非关键字（§9）
tool        := "tool" ident "{" fn* "}"
actor       := "actor" ident "{" (ident ":" field+)? ("message" ident ("{" field* "}")? | on_clause)* "}"   // §87 #5；状态段标签（约定 state）为普通标识符，非关键字（§9）
...
type_arg      := type | literal                // 使用侧：类型实参 或 const 值实参（如 Array<int, 3> 的 3,§86 #3）
generic       := "<" generic_param ("," generic_param)* ">"   // 泛型形参列表（声明侧）；编译期 bound 经 where_clause（§86 #2），不用内联 `T: Trait`
generic_param := ident | "const" ident ":" type              // 声明侧：type 形参（T）或 const 形参（const N: int,§86 #3）
block       := "{" stmt* "}"
...
> `List` / `Map` / `Set` 为 **COW 值类型**（§18.4、§85 #2/#7）：赋值=共享缓冲（**共享句柄，非位复制** → Copy-compatible，计入 `is_copy`；**原子 rc** → `Send + Sync`），修改=克隆；行为同 Copy，**无追踪 GC**。可变方法（`insert`/`append`/`remove`）声明 `borrow_mut self`。
...
```ail
interface Model {
    fn generate(borrow self, prompt: string) -> Future<Result<Response, AIError>>   // sync 方法：borrow self 仅存同步调度帧、不跨 await（§18.5 / §74.2）；返回的 Future 捕获 owned 请求数据
    effects { network }
}
```
...
{
    let id: UserId = req.param<UserId>("id")        // return-only 显式（§86 #7 / §84 #14）
...
extern_fn   := "fn" ident "(" params? ")" ("->" type)?
type        := ident ( "<" type_arg ("," type_arg)* ">" )?
type_arg    := type | literal     // 使用侧：类型实参 或 const 值实参（如 Array<int, 3>,§86 #3）
block       := "{" stmt* "}"
stmt        := "let" ident (":" type)? "=" expr
            | "var" ident (":" type)? "=" expr
            | "return" expr?
...
- **`Send`/`Sync` auto-trait 推导**（§87 #6）：编译器按字段组合自动派生 `Send`/`Sync`（marker lang item，§86 #5）——Copy→Send+Sync、COW（原子 rc）→ Send+Sync、Move 资源→Send 非 Sync、`Mutex<T>`/`Shared<T>` 按规则；`spawn`/`Channel.send`/actor 投递边界静态查 `Send`（类 Rust，无需手写 `impl`）。
...
| `float` | `f64` |
| `bool` | `i8`（存储）/ `i1`（寄存器）|
| `string` | `{i64 len, i8* ptr}`（COW：共享缓冲 + **原子引用计数** → `Send + Sync`，修改时克隆；§85 #2） |
...
| `enum` | tag + data |
| `Future<T>` | 状态机 struct（`async fn` 编译产物）；**无 `Pin`**——borrow 禁跨 await（§18.5 / §74.2）→ 不自引用（§87 #1） |
...
| **1** | 1–3 月 | Lexer、Parser、AST、基础类型、函数、struct、**最小 LLVM 输出**、**所有权子集**（move/UAM/确定性释放 §18.7/函数内 borrow/borrow_mut aliasing §74.2，非泛型） | `fn main() { print("hello") }` 编译为原生二进制并运行；use-after-move 报错 |
| **2** | 3–6 月 | `Result`、`enum`、泛型、`interface`、**AI Metadata（`.ailmeta`）** | 端到端：源码 → 可信 AI 元数据 |
| **3** | 6–12 月 | **完整 Borrow Checker**（泛型所有权 + Lifetime Analyzer 跨函数推导，Phase 1 已交付所有权子集）、并发、标准库核心 | 所有权错误用例全部报出 |
| **4** | 1 年+ | 包管理、IDE 插件、Debugger、AI Agent 生态 | 生产级 |

> **关键 checkpoint**：Phase 1 打通全工具链（原生 hello world）+ 所有权子集（move/UAM/确定性释放）；Phase 2 交付 `.ailmeta`，验证 AI-native 主张；Phase 3 补齐**完整** Rust 级 borrow checker（泛型 + Lifetime Analyzer）。
...
> **borrow checker 增量交付**（原「临时内存模型」开放缺口，**2026-07-13 已定**）：**无临时补丁**——Phase 1 即交付**所有权子集**（move 语义 + use-after-move 检测 §74.1 + 确定性释放 §18.7 + 函数内 borrow/borrow_mut aliasing §74.2，非泛型），Phase 3 扩展为**完整 borrow checker**（泛型所有权规则 + Lifetime Analyzer 跨函数推导 §18.5，依赖 Phase 2 泛型/trait 系统）。排除 RC（move 退化违 §18.3）/ unsafe（违编译期安全卖点）。每阶段语义为 §18 真子集、永久正确（见 §82 #1）。
...
14. `[语义]` **`Request.param<T>(name: string) -> T`**（HTTP 边界解析 + 强制，return-only 泛型）；`let id: UserId = req.param<UserId>("id")`（调用点显式，§86 #7）。=> 体现在 §63。
...
2. `[定义]` **COW 原子引用计数 → `Send + Sync`**（原子 rc 使共享只读线程安全，写时克隆线程本地）；「无 GC」修订为「无追踪 GC；COW/RC 引用计数仍存，确定性释放」。=> 体现在 §18.4 / §18.6。
...
1. `[语义/矛盾]` **`async fn f() -> T` 的 `T` 即 `await` 值**（`Future<T>` lang item）；「须返回 Result」仅约束用户显式 `await` 的 async fn，runtime 派发 handler 豁免；**状态机无 Pin**（borrow 禁跨 await，规则在 §18.5 / §74.2）。`task` 不可 async（void body + CSP 通道，经 channel/cancel 通信）。=> 体现在 §21。
...
6. `[定义]` **`Send`/`Sync` 为自动派生 marker trait（lang item）+ 组合规则**：`Send` 当且仅当所有字段 `Send`、`Sync` 当且仅当所有字段 `Sync`；Copy → Send+Sync、COW → Send+Sync、Move 资源 → Send 非 Sync、`Mutex<T>`/`Shared<T>` → Send+Sync。=> 体现在 §18.6。
...
8. `[一致性]` **并发错误传递统一表**：`async fn -> Result` 可 throw/try；`task` 禁 throw（经 channel/cancel）；actor `on` 禁 throw（经 `reply(<错误变体>)`，reply 值可 Result）；server handler ✗ 禁 throw（豁免于须返回 Result，runtime 派发）。=> 体现在 §21。
...
10. `[一致性/完整性]` **`type X = Y` 复杂/不透明体 kind**：无 meaning/constraint = alias（base 可不透明，记规范化类型串）；附 meaning 或 constraint = semantic；`...` 仅示例简写非真实语法。=> 体现在 §15.3。
...
3. `[文法/决断]` **§27 spawn 引用矫正** `call` → `postfix`：spawn 产生式 `( call | block )` 的 `call`（§27 定义为纯调用后缀 `(<…>) | (args)`，不含被调者）无法归约 `spawn download()` / `spawn actor Name()`（须 `primary + call = postfix`），与 §21/CONCURRENCY「均合法（§27）」矛盾——前四轮查可达性漏此引用错。改 `( postfix | block )`（与 `send := postfix "!" expr` 一致），§71 镜像同步。代价：`postfix` 允许 `spawn x.y()`（方法引用 spawn，合理）、裸值（语义分析拒，骨架可忍）——合理过生成；`call` 仍被 `postfix` 引用，零悬空不破。=> 体现在 §27 / §71。
...
10. **`spawn` 三形式锁定**：`spawn <name>(args)`（具名 Task）/ `spawn actor Name(args)`（Actor，返 `ActorHandle`，§87 #5）/ `spawn { block }`（匿名块）均合法（§21、§27 文法）。
...
### v0.2.1 治理地基补齐（P0-0，2026-07-27）

> 依据两轮深度评审综合判断（`docs/research/synthesis-2026-07.md`），P0-0「规范治理地基」是其余一切补齐的前提。本次**不改动既有语义决断、不新增关键字、不新增文法产生式**，仅补齐规范工程元规则层；**版本号维持 v0.2.1 不变**（治理补齐不构成语义破坏性变更，统一演进留待 P0 路线完成时）。

13. **新增 §1.5 Conformance、规范性词汇与行为分类**：RFC 2119 中文映射（必须 / 不得 / 应 / 可以）、normative / informative 分区、合规实现定义、定义行为 / 未定义行为(UB) / 实现定义 / 未指定 四分法、build profile（debug / release）规范语义。该节为 §2–§94 的元前提。
14. **新增附录 D 术语表（Glossary）**：规范性词汇与关键术语的资料性索引。
15. **新增独立 RFC lifecycle 文档** `docs/rfc-process.md`：定义 RFC 从 Draft 到 Implemented 的生命周期与采纳标准（规范 §1.5 引用）。
16. **附录 B 登记「表达式求值顺序未指定」为遗留 #7**（计划 P0-2 收紧为左到右）——本次仅登记现状，未改动求值语义。
...
8. ✅ **`spawn` 三形式锁定**：`spawn <name>(args)` / `spawn actor Name(args)`（§87 #5）/ `spawn { ... }` 块均合法（§21、§27）。
...
7. **表达式求值顺序未指定**（P0-0 登记 / P0-2 待收紧）：实参、操作数、字面量元素的求值次序，除 `&&` / `||` 短路由外未作规定（见 §1.5.4 未指定行为）。计划在后续 P0-2 收紧为**左到右**逐产生式求值。=> 体现在 §1.5.4。
...
> 本附录是**资料性**术语索引，给出本规范关键术语的精确定义与 RFC 2119 规范性词汇映射。术语定义与正文冲突时，以正文（尤其 §1.5）为准。

### D.1 规范性词汇（RFC 2119 映射，见 §1.5.1）

| 中文 | RFC 2119 | 含义 |
|---|---|---|
| **必须** | MUST / SHALL / REQUIRED | 合规硬约束；违反即不合规 |
| **不得** | SHALL NOT / MUST NOT | 合规禁止；违反即不合规 |
| **应** | SHOULD / RECOMMENDED | 强烈建议；偏离须文档说明 |
| **可以** | MAY / OPTIONAL | 实现自由度；是否实现均合规 |

### D.2 语言与类型术语

- **语义类型（semantic type）**：带可读名称与可选约束（constraint）的具名类型别名，如 `type UserId = int`。让概念边界对 AI 与人类同时显式（§15.1 / §92）。
- **约束（constraint）**：附属于类型 / 字段的不变式断言（如 `where { self >= 0 }`），运行期由约束求值器检查（§15.4 / §92 #8）。
- **`meaning`**：附属于声明的自然语言描述（双轨：源码内联 + `.ailmeta` 结构化），供 AI 消费（§23 / §83 #10）。
- **`effect` / 效果系统**：函数副作用的显式标注（如 `effect Io`、`effect Alloc`）；pure = 无副作用且无分配（§19）。
- **所有权（ownership）**：值有唯一 owner，move 转移所有权，drop 释放（§18）。
- **move**：所有权转移语义；move 后原绑定不可用（§18.3）。
- **borrow / borrow_mut**：临时借用（只读 / 可变），不转移所有权，受生命周期与别名约束（§18.5 / §74.2）。
- **Task**：并发执行的轻量单元，由 `spawn` 创建，结构化作用域 join / cancel（§21 / §87）。
- **Actor**：带邮箱的并发实体（`spawn actor`），消息驱动（§21 / §87 #5）。
- **`.ailmeta`**：包的强制元数据产物，机器可读、AI 可消费（§24 / §32）。
- **`schema_version`**：`.ailmeta` 版本字段，当前 `0.2.0`（RFC 0001–0004 预留升至 `0.3.0`，§88 #2）。

### D.3 规范工程术语（见 §1.5）

- **合规实现（conforming implementation）**：满足 §1.5.3 全部 **必须** 项的 AILang 实现。
- **规范性（normative）/ 资料性（informative）**：是否构成合规要求，见 §1.5.2。
- **定义行为（defined behavior）**：规范完整规定结果，见 §1.5.4。
- **未定义行为（UB）**：规范不施加任何约束；**安全代码永不产生 UB**，见 §1.5.4。
- **实现定义行为（implementation-defined）**：规范要求文档化，但允许实现间不同（v0.2.1 未登记）。
- **未指定行为（unspecified）**：规范允许若干合规结果之一，不要求文档化。
- **安全代码（safe code）**：不含 `unsafe` 块 / `extern` 的代码。
- **build profile**：debug / release 构建配置，控制检测强度与优化级别（§1.5.5）。
- **panic**：确定性栈展开 + 同序 Drop 的定义性失败（§17），**不是** UB。
