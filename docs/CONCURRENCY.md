...
spawn detached compute()                // 显式分离：仍返回 TaskHandle，cancel 可传播（非 fire-and-forget，SPEC §24 #4）
```

`spawn` 时编译器检查（见 SPEC §24）：
- 参数是否 `Send`（可跨线程移动，auto-trait 推导）
- 生命周期是否安全（被 spawn 的数据不能是 `borrow` 的非逃逸借用——`borrow` 不逃逸且禁跨 `await`，权威见 SPEC §10.3 / DESIGN §9.1 / MEMORY §7；本文 §4 释其与「无 Pin」的关系）
...
- **无 `Pin`**（相对 Rust async 的关键简化）：`borrow`/`borrow_mut` **不逃逸、禁跨 `await` 点**（权威：SPEC §10.3 / DESIGN §9.1 / MEMORY §7）→ async 状态机**永不自引用** → 无需 Rust 的 `Pin`。此「禁跨 `await`」是并发各节反复引用的拱顶规则（§3、§5 借用不逃逸均其推论）。
...
- `Channel` 为运行时支持的**共享 `Send` 句柄**（跨 spawn 克隆，底层队列）；`spawn { }` 块体为**控制构造**，可捕获外层不可变 / `Send` 变量（**不含 `borrow`**——借用不逃逸，详见 §4（本文）/ SPEC §10.3 / DESIGN §9.1 / MEMORY §7；非通用闭包，v0.2 无一等闭包）——见 SPEC §13、SPEC §21 #7。
...
上图运行时生命周期为 executor 调度态，**不入** `.ailmeta`；task 的 `.ailmeta` `declarations[]` 条目（节选——省略 `span`，schema `task` 附加 `sig` 字段，SPEC §25 #4 / WHITEPAPER §3）：
```json
...
- 编译器自动**分片 → 调度线程 → 合并结果**；body 须 `pure` 或仅 `io.read`、**无共享可变**（SPEC §24 #9）（§8（本文））。
...
3. ✅ **`spawn` 三形式锁定**：`spawn task(args)` / `spawn actor Name(args)`（SPEC §24 #5）/ `spawn { ... }` 均合法（SPEC §13、SPEC §18）。
