...
| | |
|---|---|
| **状态** | 草案（Draft v1）—— 待 review（已跑多轮对抗式 workflow pass-2–pass-15，post-pass-15 唯一 finding（§6 容器字面量 `list_lit`/`map_lit` synth 规则缺席、空 `let x = []` 使 decidability 次级承诺悬空）已修正；pass-16 报 2M 并已修正（§6 rule ③ 空 `[]` check `Array<T,N>` 补 N=0 约束 + §7 方案 A 缺点补 literal `type_arg` 维度）、待 pass-17 复验；目标收敛 0H/0M/0L，对齐 RFC 0001 v6 / 0002 v8 / 0003 v5 / 0004 v5 / 0005 v1）|
