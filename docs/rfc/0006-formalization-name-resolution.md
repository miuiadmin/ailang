...
| | |
|---|---|
| **状态** | 草案（Draft v1）—— 待 review（已跑多轮对抗式 workflow pass-2–pass-15，post-pass-15 唯一 finding（§6 容器字面量 `list_lit`/`map_lit` synth 规则缺席、空 `let x = []` 使 decidability 次级承诺悬空）已修正、待 pass-16 复验；目标收敛 0H/0M/0L，对齐 RFC 0001 v6 / 0002 v8 / 0003 v5 / 0004 v5 / 0005 v1）|
...
**收敛轨迹**：已跑多轮对抗式 workflow（pass-2 → pass-14，与 RFC 0005/0007 同批 pipeline，每轮修正后 FRESH 重跑、无 `resumeFromRunId`）。历轮累计修正：pass-2 报 1H/5M/5L（11 条，含 §3 Conformance 重定向后的回归扫描、`<` 消歧方案 B 迁移清单、EBNF 产生式自洽、decidability 论证）；pass-6/pass-10/pass-11 多轮 M/L（类型导向 variant 解析、`Err` 无包装、actor message 解析等）；**pass-14 报 2L 并已全部修正**：① §6 rule 6 block 尾定型示例 match arm 误用胖箭头 `=>` → 改冻结 §16/§27 锁定的 `Pattern: { body }` 形；② §8/§9 `WebSearch.query` 引用由「§23 / §60」收窄至**仅 §60**（§23.1/§40.2 的 `web.search` 形与自身 `tool Search` 声明不一致、为本 RFC 点分路径规则不覆盖的冻结残留示例）。post-pass-14 待 pass-15 复验收敛。**pass-15 报 1L 并已修正**（0005/0007 本 pass clean、0 confirmed）：§6 rule 4 仅枚举 §5 `literal` 内**标量**字面量（`int_lit`/`float_lit`/`char_lit`/`string_lit`/`true`/`false`），遗漏 §27 `primary`（line 1535）另两个终结符 `list_lit`/`map_lit`——空容器 `let x = []` 元素类型 `T` 无源（无元素可 synth）、既非唯一类型亦非已报错误，使 §6 decidability 声明「总是产生唯一类型或报告类型错误」对该边界 case 悬空。修正=§6 rule 4 增补容器字面量 synth/check 子规则：① 非空 `[e₁,…,eₙ]` synth 各元素并合一（不一致报错）、容器种类默认 `List<T>`（§15.9 line 604）；② 非空 `[k₁:v₁,…]` synth 键/值分别合一；③ **空容器** check 上下文从期望类型取元素类型（`check([], List<int>)`→`T=int`），synth 上下文无期望（`let x = []`）**报告类型错误**「需类型标注确定元素类型」（对齐 Rust「type annotations needed」），闭合 decidability 悬空。post-pass-15 待 pass-16 复验。审查维度：EBNF 产生式自洽 / 词法优先级无冲突 / 推断算法 decidability 证明严谨性 / `<` 消歧两方案 tradeoff 完备性 / 名字解析与 §89 §92 §12.2 对齐 / 与 §16 §87 §86 既有语义无矛盾 / 关键字地位 / AI 可消费性。
