...
1. `a` 按简单名解析（值或类型或模块）——**tie-break**：当 `a` 在同一作用域层**同时命中值与类型命名空间**时（合法：`struct Foo`/`type Foo` 与 `fn Foo`/`let Foo` 同名、命名空间分离、声明层无遮蔽，见上「命名空间」），**值命名空间优先**（与下文 `Name(args)` tie-break「值命名空间优先」对称——点分路径为值位表达式、取值域绑定；类型域同名绑定在类型位上下文仍可达、不被值遮蔽，此 tie-break 仅决断值位点分路径的引用层选择、不改变声明层分离）；
