...
- **数字串起始位 `_` 承诺（词法错误模型扩展）**：与上述三条同形，浮点字面量内 `_` 出现在文法要求数字**首位**处（指数标记 `e`/`E` 紧后、指数符号 `e+`/`e-` 紧后、小数点 `.` 紧后——即 `digits := [0-9] ("_"? [0-9])*` 首位强制 `[0-9]`、`_` 不得开启数字串的位置），如 `1e_5`、`1._5`、`1e+_5`、`1e-_5`，为**词法错误**（AIL1xxx）。承诺依据：`digits` 首位不变量为 `[0-9]`（非 `_`），`_` 在此位置出现时虽后随数字、却前随非数字（`e`/`E`/`+`/`-`/`.`）违「`_` 须在同一数字串内前后均随数字」的实际不变量；**不**回退为「字面量 + `ident`」拆分——否则 `1e_5` 会被 maximal munch 拆为 `int 1` + `ident e_5`（`e_5` 匹配 §8.6 标识符 `[A-Za-z_][A-Za-z0-9_]*`）、`1._5` 拆为 `int 1` + `.` + `ident _5`，延后报令人困惑的「相邻表达式」错。此条款把数字分隔符的**起始位**半边落地为与「前导零」「尾部 `_`」「连续 `_`」同形的词法错误模型，闭合第四边。注：`0x`/`0o`/`0b` 紧后 `_`（如 `0x_FF`）已由前述进制前缀承诺覆盖（识别前缀即承诺进制、紧后非有效数字报词法错），不在本条范围。注：本 RFC `decimal := [1-9] ("_"? [0-9])*` 与 Python 文法 `\d(_?\d)*` 同构，Python 亦拒绝 `1__0`（`SyntaxError`）——本条款使 AILang 在**词法层**即以清晰错误拒绝、而非如未补时延后报 parser 困惑错，契合承诺模型「词法层消除延后 parser 困惑错」的设计目的。
...
1. `a` 按简单名解析（值或类型或模块）——**tie-break**：当 `a` 在同一作用域层**同时命中值与类型命名空间**时（合法：`struct Foo`/`type Foo` 与 `fn Foo`/`let Foo` 同名、命名空间分离、声明层无遮蔽，见上「命名空间」），**值命名空间优先**（点分路径为值位表达式、取值域绑定；此 tie-break 仅决断值位点分路径的引用层选择、不改变声明层分离）。**与下文 `Name(args)` tie-break〔line 281〕的辨析**：二者冲突空间不同——`Name(args)` 类型侧的 call 形仅有 constraint `T(expr)`〔line 276〕与 Builtin 位置构造器 `Name<T>(args)`〔line 277〕、enum/struct/alias 类型整体**无** `EnumType(args)` 构造形（variant 经 line 278 路径形 / 裸名形构造），故 line 281 的冲突是 fn vs constraint 二者**皆具 `(args)` 形**的真歧义、值优先阻断的是 constraint 构造；而本 step 1 的类型侧有有效 dotted member（enum variant〔line 267 `Color.Red`〕），冲突是 value-field vs type-variant——值优先阻断的是 variant 构造。**后果登记**：当 `a` 同时为 enum/error 类型与值域绑定（合法同名、如 `enum Color { Red }` + `const Color = 5`）时，值位点分路径 `a.b`〔如 `Color.Red(args)`——line 278 标准 variant 构造形 / 冻结 §44 line 2044 `Enum.Variant`〕经 step 1 路由到值域绑定后、step 2 line 266 在该值上查 `b` member——基础类型无 variant 字段 → `UnresolvedMember`；类型域解释（`a` 为 enum、`.b` 为 variant）已在 step 1 被阻断、不可达。**无点分路径逃生口**：无 `Self` 类型关键字〔冻结 §9 line 379 仅小写 `self` 接收者参数〕、turbofish `::<T>`〔§7 方案 B〕仅附加于泛型可调用实体而 enum 非 generic、type-directed 解析〔第四机制 line 247-253〕仅解析裸 variant 名不解析路径头 `a`。**逃生路径**：① 在类型标注上下文用裸 variant 名〔如 `let x: Color = Red(args)`、type-directed 解析 line 251 ③〕；② 改名消歧〔如 `fn make_color` / `enum Color`、与 line 281 fn+constraint 同名「须改名消歧」处理范式对称〕。**soundness 声明收窄**：类型域同名绑定仅在**类型位 bare ident 上下文**〔如 `let x: Color`〕可达、不被值遮蔽；值位点分路径引用类型域 member（variant 构造）在 value+enum/error 同名下不可达、须上述逃生路径。此为病态罕见场景（enum/error+value 同名），值优先最大化点分路径可预测性、不值得为它引入回退 tie-break 或新语法；
...
| §8 step1 点分路径 value+type 同名 tie-break（值优先；value+enum/error 同名下 `EnumType.variant` 点分路径不可达、须改名或类型位裸名） | §27 `a.b` dotted-path + §44 line 2044 `Enum.Variant` 标准形 + §9 line 379 无 `Self` 关键字 + §7 turbofish 仅泛型可调用实体 + line 251 ③ type-directed 裸名 | ✅ 诚实登记（与 line 309 turbofish 非逃生口「fn+constraint 须改名消歧」范式对称、但冲突空间不同：line 309/281 类型侧仅 constraint call 形、本处类型侧有 variant member；逃生=类型位裸名 type-directed〔line 251 ③〕/ 改名消歧） |
