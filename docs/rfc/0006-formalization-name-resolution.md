...
| | |
|---|---|
| **状态** | 已接受（Accepted v1）—— 2026-07-28 Accepted〔RFC 0005 §9 line173 采纳标准全满足：① 自洽（与 §1.5/§17/§90 无矛盾）② 对抗验证收敛 0H/0M/0L ③ 冻结影响声明 ④ 实现可行性；Review 经对抗式 workflow 完成、FCP 折叠（solo 治理）；不触动冻结 v0.2.1、Implemented 阶段合并进 AILANG.md/编译器时生效〕。收敛证据（已跑多轮对抗式 workflow pass-2–pass-19，post-pass-15 唯一 finding（§6 容器字面量 `list_lit`/`map_lit` synth 规则缺席、空 `let x = []` 使 decidability 次级承诺悬空）已修正；pass-16 报 2M 并已修正（§6 rule ③ 空 `[]` check `Array<T,N>` 补 N=0 约束 + §7 方案 A 缺点补 literal `type_arg` 维度）；pass-17 / pass-18 报 0 confirmed（持续收敛、无回归）；pass-19 报 1L（§6 decidability 声明 α gloss「≈ 近线性」与 α 标准定义反阿克曼函数近常数矛盾、致 O(n·α(n)) 量级内部矛盾）已修正；pass-20 报 0 confirmed（本 RFC clean）；**pass-21 报 1 confirmed [0H/0M/1L]**（name-resolution-alignment——§8 点分路径 `a.b` 步骤 1 缺与 `Name(args)`（line281）对称的 tie-break：`a` 同时命中值/类型命名空间时无决断）已修正（步骤 1 补「值命名空间优先」tie-break）；**pass-22 / pass-23 复验 0006 clean**（findings 分别落 0005 / 0007）；**pass-24 报 1 confirmed [0H/1M/0L]**（name-resolution-alignment——步骤 1 tie-break 的次级后果：value+enum/error 同名下值优先使 `EnumType.variant` 点分路径〔line 278 标准 variant 构造形 / 冻结 §44 line 2044〕经 step 1 路由到值域后 step 2 line 266 查 member 失败→`UnresolvedMember`、类型域 variant 解释被阻断不可达；原「类型位上下文仍可达」soundness 声明过宽〔仅 bare ident 可达、dotted variant 不可达〕、「与 line 281 对称」claim 结构不精〔冲突空间不同：line 281 类型侧仅 constraint call 形、本处类型侧有 variant member〕）已修正（步骤 1 补后果登记 + 逃生路径〔类型位裸名 type-directed / 改名消歧〕+ soundness 收窄 + 与 line 281 辨析、§9 自洽核查表补登记行）→ **pass-25 报 2 confirmed [0H/0M/2L] 并已修正**（① lex-priority——§5 承诺模型覆盖前导零/裸尾 `_` 但缺连续 `_`〔`1__0` 静默拆为 `int 1` + `ident __0`、与裸尾 `_` 同构、未补时延后报 parser 困惑错恰承诺模型设计来消除的那类〕→ §5 补「连续 `_` 承诺」条款〔`_` 后须紧跟数字否则 AIL1xxx、不回退拆分〕三边闭合；② semantic-no-conflict——§6 line173 + §9 line294/300「§4 loop 移出 expr 降为语句」蕴含 loop 先前在 expr、与 §4/§11#1/§9 自洽列「未入 expr 产生式」矛盾〔§9 line294 同行两列自相矛盾标✅〕、且暗示改冻结 §27 → 四处改「冻结 §27 loop 仅在 stmt、从未入 expr、§4 维持此现状、非演进」、与冻结 §27 loop 在 stmt〔line1515、expr 链 line1520-1535 不含〕一致）；**pass-26 报 1 confirmed [0H/1M/0L]**（lex-priority——pass-25 补「连续 `_` 承诺」后声明「三边闭合、承诺模型覆盖边界与其自身依据〔`_` 作分隔符须后随数字〕一致」为假声明：存在第四类 `_` 边界——`_` 在文法要求数字首位处〔指数标记 `e`/`E` 紧后、指数符号 `e+`/`e-` 紧后、小数点 `.` 紧后；即 `digits := [0-9] ("_"? [0-9])*` 首位强制 `[0-9]`〕如 `1e_5`/`1._5`/`1e+_5`/`1e-_5` 虽后随数字却前随非数字违实际不变量、三条承诺无一覆盖、经 maximal munch 静默拆为 `int` + `ident` 延后报 parser 困惑错；所述依据「须后随数字」是半真理、实为「须在同一数字串内前后均随数字」）已修正（§5 补第四边「数字串起始位 `_` 承诺」条款〔起始位 `_` 为词法错误 AIL1xxx、不回退拆分〕+ 原「三边闭合」改「四边闭合」+ 依据加宽为「前后均随数字」）；**pass-27 报 2 confirmed [0H/1M/1L] 并已全部修正**（[M] lex-priority——pass-26 补第四边「起始位 `_` 承诺」后声明「四边闭合」对此类为假：`float_lit` 经 alt1 `decimal "." digits exp?`（含小数部）成功匹配后、`exp?` 因 digits 首位遇 `_` 失败被视 absent、`1.0` 自然成功 + 剩余 `e_5` 当下一 ident（`1.0e_5`/`3.14e_5` 静默拆分为 `float + ident`、与 `1e_5` 用户意图同构却仅后者被捕获）→ §5 补强：float alt1 成功后 lexer 须 lookahead `e/E (+|-)? _` 模式、若是则报 AIL1xxx 不产 `float + ident` 拆分（`1.0e_5`/`1.0E_5`/`1.0e+_5`/`1.0e-_5` 均归此条款、与 `1e_5` 同判）、「四边闭合」覆盖全部 digits 首位 `_` 位置；[L] name-resolution-alignment——§8 step1 tie-break + §9 自洽核查表的〔line XXX〕内部交叉引用随 §5 增补系统性 +2 偏移（pass-25/26 §5 增「连续 `_` 承诺」+「第四边」条款后未同步行内引用、line 281→283/line 276→278/line 247-253→249-255 等）→ 改语义引用〔§8 类别消歧段 / step2 分支 / type-directed 解析段 等〕根治行号漂移、与冻结 §44/§9 line 引用区分保留）已全部修正；**pass-28 报 0 confirmed = CONVERGED 0H/0M/0L**（1 raw finding ai-consumability〔§6 rule6 函数体 block 尾类型规则「尾表达式 / `;` 收尾」区分〕被 default-refute verify 驳回——rule6 完备三分〔尾表达式→synth/check 类型、无尾表达式（以 `;` 收尾）→void、尾为函数级控制流出口 throw/return→never〕可由「`;` 在则 void、不在则尾表达式」直接逻辑推导、§8.3「等价」仅以 `let` 语句演示分隔符角色被 finding 过度泛化至末位裸 expr 的 tail-discard-marker 角色、`;` 双重角色〔语句间可选分隔符 vs 末位 expr 尾/非尾语义标记〕为 block-expression 语言通行惯例、rule6 为类型规则非文法规则〔RFC 未改 §27 block/stmt 产生式〕、AST trailing-semicolon 元数据系实现层非规范层）= batch A（0005/0006/0007）收敛、达成 0H/0M/0L）|
...
- **连续 `_` 承诺（词法错误模型扩展）**：与上述前导零承诺 / 尾部 `_` 承诺同形，数字字面量内 `_` 后**须紧跟数字**——`_` 后紧跟另一个 `_` 或其他非数字字符（连续 `_`，如 `1__0`、`0xFF__1`、`1.0__5`、`1e0__5`）为**词法错误**（AIL1xxx）。承诺依据：词法在识别数字字面量时已承诺其内 `_` 为分隔符（「`_` 不可在首尾」§5 数字分隔符规则——其依据为 `_` 作分隔符须后随数字），连续 `_` 中第一个 `_` 已被词法尝试消费为分隔符、却未后随数字（紧接第二个 `_` 违约），与裸尾 `_` 同构；**不**回退为「字面量 + `ident`」拆分——否则 `1__0` 会被 maximal munch 拆为 `int 1` + `ident __0`（`__0` 匹配 §8.6 标识符 `[A-Za-z_][A-Za-z0-9_]*` 允许 `_` 起首）、`0xFF__1` 拆为 `int 0xFF` + `ident __1`、`1.0__5` 拆为 `float 1.0` + `ident __5`，延后报令人困惑的「相邻表达式」错。此条款把数字分隔符的**内部**半边（`_` 后须为数字、不得为另一 `_`）落地为与「前导零」「尾部 `_`」同形的词法错误模型——前导侧由前导零承诺覆盖、尾部侧由尾部 `_` 承诺覆盖、内部侧由本条覆盖、起始位由下条「数字串起始位 `_` 承诺」覆盖，四边闭合、承诺模型覆盖边界与其自身依据（`_` 须在同一数字串内前后均随数字——既不可裸尾亦不可前随非数字）一致。
- **数字串起始位 `_` 承诺（词法错误模型扩展）**：与上述三条同形，浮点字面量内 `_` 出现在文法要求数字**首位**处（指数标记 `e`/`E` 紧后、指数符号 `e+`/`e-` 紧后、小数点 `.` 紧后——即 `digits := [0-9] ("_"? [0-9])*` 首位强制 `[0-9]`、`_` 不得开启数字串的位置），如 `1e_5`、`1._5`、`1e+_5`、`1e-_5`，为**词法错误**（AIL1xxx）。承诺依据：`digits` 首位不变量为 `[0-9]`（非 `_`），`_` 在此位置出现时虽后随数字、却前随非数字（`e`/`E`/`+`/`-`/`.`）违「`_` 须在同一数字串内前后均随数字」的实际不变量；**不**回退为「字面量 + `ident`」拆分——否则 `1e_5` 会被 maximal munch 拆为 `int 1` + `ident e_5`（`e_5` 匹配 §8.6 标识符 `[A-Za-z_][A-Za-z0-9_]*`）、`1._5` 拆为 `int 1` + `.` + `ident _5`，延后报令人困惑的「相邻表达式」错。此条款把数字分隔符的**起始位**半边落地为与「前导零」「尾部 `_`」「连续 `_`」同形的词法错误模型，闭合第四边。**补强（覆盖 float 含小数部后 exp digits 缺位 `_`）**：上述机制覆盖 `float_lit` 整体失败回退 `int_lit` 形（`1e_5` 经 alt2 `decimal exp` 的 exp digits 首位遇 `_` 整体失败 / `1._5` 经 alt1 `decimal "." digits` 的 `.` 后 digits 首位遇 `_` 整体失败），但存在第三类：`float_lit` 经 alt1 `decimal "." digits exp?` 成功匹配（含小数部）后、`exp?` 因 digits 首位遇 `_` 失败而被视为 absent 的形（如 `1.0e_5`：`decimal`=`1`、`.`、`digits`=`0` 三者成功 → `exp?` 尝试 `e` 消费后 digits 首位遇 `_` 失败 → `exp?` 视为 absent → alt1 成功匹配 `1.0`〔**非回退**〕→ token 为 `float(1.0)`、剩余 `e_5` 为下一 `ident`——此过程无「回退为字面量+ident」发生、上述「不回退」机制不触发、且 `_` 位于 float `1.0` 之外 scope「浮点字面量内」亦不覆盖）；用户意图同构（`e` 紧后 `_`、意图指数 `_` 分隔违 digits 首位 `[0-9]` 不变量），却仅 `1e_5`（alt2 失败）被捕获、`1.0e_5`（alt1 成功）逃逸。**补强机制**：词法在 `float_lit` 经 alt1（含小数部）成功匹配后，须 **lookahead** 检查紧后源码是否为 `e`/`E` (`+`|`-`)? `_` 模式（即用户意图指数 `_` 分隔、违 exp digits 首位 `[0-9]` 不变量）；若是，则报 **AIL1xxx 词法错误**、不产 `float + ident` 拆分（`1.0e_5` / `3.14e_5` / `1.0E_5` / `1.0e+_5` / `1.0e-_5` 均归此条款、与 `1e_5` 同判 AIL1xxx）。此补强使「数字串起始位 `_` 承诺」覆盖**全部** digits 首位 `_` 位置（整数 digits 首位 / 小数 digits 首位 / 指数 digits 首位——无论 float 是否整体成功），「四边闭合」声明对此类为真。注：`0x`/`0o`/`0b` 紧后 `_`（如 `0x_FF`）已由前述进制前缀承诺覆盖（识别前缀即承诺进制、紧后非有效数字报词法错），不在本条范围。注：本 RFC `decimal := [1-9] ("_"? [0-9])*` 与 Python 文法 `\d(_?\d)*` 同构，Python 亦拒绝 `1__0`（`SyntaxError`）——本条款使 AILang 在**词法层**即以清晰错误拒绝、而非如未补时延后报 parser 困惑错，契合承诺模型「词法层消除延后 parser 困惑错」的设计目的。
...
4. **字面量类型确定**——无后缀字面量（§5）的 `synth`：`int_lit` → 默认 `int`（i64），但若处 `check(T)` 上下文且 `T` 为整数类型（`uint`/`int8`..），则采用 `T`（双向消歧）；**`T` 为整型 base 的 semantic / constraint 类型时**（如 `UserId`、`type Age = int + meaning`，§15.3 / §15.4），`int_lit` 经**字面量强制**（§15.3 / §84 #1）跨 nominal 归属 `T`——此为 coercion（单向字面量→semantic，作用于**全部 check 位**：`let` 绑定 + 调用实参，非仅 `let`；变量间仍禁隐式转换，§15.3 nominal 不变 §86 #9），与上方原始整型宽度收窄为不同机制；`float_lit` → 默认 `float`（f64），可被 `check(f32)` 收窄（宽度收窄，与整型同机制）；**不**扩展字面量强制到 float-base semantic 类型——冻结 §15.3 / §84 #1 / §92 #7 的字面量强制授权**仅以整数为例**，本 RFC 不擅自对称扩展到浮点（float-base semantic 的字面量构造留后续 RFC）；`char_lit` → `char`（Unicode 标量值类型，由 [RFC 0007](./0007-determinism-batch.md) §8 引入 §15.1——本 RFC 定义 `char_lit` 的**词法**、其**类型归属** `char` 跨 RFC 由 0007 §8 闭合，两 RFC 同批落地故自洽）；`string_lit` → `string`；`true`/`false` → `bool`。**容器字面量（`list_lit` / `map_lit`）的 synth/check**——上述仅枚举 §5 `literal` 内的**标量**字面量；`list_lit` / `map_lit` 为 §27 `primary`（line 1535）的另两个终结符、**不在** §5 `literal`（line 136）内，其类型规则单列：① **非空 `list_lit` `[e₁,…,eₙ]`**——**synth 上下文**：对每个元素 `synth(eᵢ)` 并**合一**全部元素类型（必须一致，否则报告类型错误），容器**种类**默认 `List<T>`（冻结 §15.9 line 604「无标注默认 List」、line 3446 锁定），synth 结果 `List<synth(e₁)>`；**check 上下文**（期望 `List<T>` / `Array<T, N>` / `Set<T>`、种类须匹配 §15.9 line 606 判别）：对每个元素 `check(eᵢ, T)`——**逐元素 check** 使整型 / 浮点字面量经规则 4 的 check-位**字面量强制**收窄到期望元素类型（`check([1, 2, 3], List<uint>)` 各 `int_lit` → `uint`、与标量 `let x: uint = 1` 同机制；非字面量元素 synth 后与 `T` 合一），容器种类由期望约束（`check([1, 2, 3], Set<int>)` → `Set<int>`）；`Array<T, N>` 另须元素计数 = `N`（`check([1, 2, 3], Array<int, 3>)` ✓ / `Array<int, 5>` ✗ 类型错）。② **非空 `map_lit` `[k₁:v₁,…,kₙ:vₙ]`**——**synth 上下文**：对每个键 `synth(kᵢ)` 合一、每个值 `synth(vᵢ)` 合一（键类型全体一致、值类型全体一致，否则报告类型错误），synth 结果 `Map<synth(k₁), synth(v₁)>`；**check 上下文**（期望 `Map<K, V>`、种类须匹配）：对每个键 `check(kᵢ, K)`、每个值 `check(vᵢ, V)`（键 / 值各自逐项 check、字面量强制同 ①）。③ **空容器 `[]` / `[:]`**——元素类型**无源**（无元素可 synth）；**种类须匹配**（§27 line 1536-1537 文法形态分离、§15.9 line 604-605 容器分类）：`[]`（`list_lit`、无冒号）仅可 check **元素集合类** `List<T>` / `Array<T, N>` / `Set<T>`，`[:]`（`map_lit`、冒号形）仅可 check `Map<K, V>`——跨类（`check([], Map<…>)`、`check([:], List<…>)`）为**类型错误**。**另 `[]` check `Array<T, N>` 须 `N=0`**——空字面量元素计数为 0、仅可满足 const 泛型 `N=0`（§15.9 line 601 `Array<T, const N: int>` 固定内联布局、§86 #3）；`N` 非 0 的期望（`check([], Array<int, 5>)`）为**类型错误**（保守拒绝——空字面量无元素可填 `N>0` 的内联布局、否则成 codegen 不初始化内存的 UB 入口），与 rule ① 非空 `Array<T, N>` 须元素计数 = `N` 对称（计数 0 ↔ `N=0`）。**`check` 上下文种类匹配时**（`let x: List<int> = []`、调用 `f([])` 形参为 `List<int>`、`let m: Map<int, string> = [:]`）从期望类型取元素类型（`check([], List<int>)` → `T=int`、`check([:], Map<int, string>)` → `K=int, V=string`），synth 结果即期望类型；**`synth` 上下文无期望类型**（`let x = []`、`let x = [:]`）——元素类型变量无任何约束、既非唯一类型亦非已报告错误，**报告类型错误**「需类型标注确定元素类型」（对齐 Rust「type annotations needed」诊断）——此条款闭合本 §6 decidability 声明「总是产生唯一类型或报告类型错误」对空容器边界 case 的悬空（否则 `let x = []` 会得到 `List<?T>` 既非唯一类型、又未报错、与 decidability 次级承诺冲突）。
...
- **缺点**：保留理论歧义边界——`f < g > (h)` 这类「比较链后跟括号」会被误解析为泛型调用（Rust pre-2018 的已知陷阱）；需 parser 维护一个试探回溯或优先级规则。对 AI 不够「可预测」。**该陷阱在 AILang 下严格宽于 Rust pre-2018**：因 §27 `type_arg := type | literal`（const 值实参，§27 line 1507、§15.9 line 601 `Array<T, const N: int>`、§86 #3 自 v0.2.1 即冻结），比较链右操作数可为**整型字面量**——`x < 5 > (y)` 中 `5` 为合法 `type_arg`（literal）、`> (y)` 收尾匹配 `type_args "(" ` lookahead，同样被误解析为泛型调用；Rust pre-2018 的陷阱仅覆盖**类型名**操作数（`f < g > (h)`），AILang 的歧义边界额外延伸到**全部整型字面量比较**（`a < 0 > (b)`、`i < 1 > (j)` 等）及括号场景——故方案 A 的歧义边界**严格宽于**所引 Rust pre-2018 基线。
...
1. `a` 按简单名解析（值或类型或模块）——**tie-break**：当 `a` 在同一作用域层**同时命中值与类型命名空间**时（合法：`struct Foo`/`type Foo` 与 `fn Foo`/`let Foo` 同名、命名空间分离、声明层无遮蔽，见上「命名空间」），**值命名空间优先**（点分路径为值位表达式、取值域绑定；此 tie-break 仅决断值位点分路径的引用层选择、不改变声明层分离）。**与下文 `Name(args)` tie-break〔§8 类别消歧段〕的辨析**：二者冲突空间不同——`Name(args)` 类型侧的 call 形仅有 constraint `T(expr)`〔§8 类别消歧·约束类型条〕与 Builtin 位置构造器 `Name<T>(args)`〔§8 类别消歧·Builtin 条〕、enum/struct/alias 类型整体**无** `EnumType(args)` 构造形（variant 经 §8 类别消歧·variant 条路径形 / 裸名形构造），故该 `Name(args)` tie-break 的冲突是 fn vs constraint 二者**皆具 `(args)` 形**的真歧义、值优先阻断的是 constraint 构造；而本 step 1 的类型侧有有效 dotted member（enum variant〔step 2「`a` 为类型」分支 `Color.Red`〕），冲突是 value-field vs type-variant——值优先阻断的是 variant 构造。**后果登记**：当 `a` 同时为 enum/error 类型与值域绑定（合法同名、如 `enum Color { Red }` + `const Color = 5`）时，值位点分路径 `a.b`〔如 `Color.Red(args)`——§8 类别消歧·variant 标准构造形 / 冻结 §44 line 2044 `Enum.Variant`〕经 step 1 路由到值域绑定后、step 2 在该值上查 `b` member——基础类型无 variant 字段 → `UnresolvedMember`；类型域解释（`a` 为 enum、`.b` 为 variant）已在 step 1 被阻断、不可达。**无点分路径逃生口**：无 `Self` 类型关键字〔冻结 §9 line 379 仅小写 `self` 接收者参数〕、turbofish `::<T>`〔§7 方案 B〕仅附加于泛型可调用实体而 enum 非 generic、type-directed 解析〔第四机制·§8 类型导向段〕仅解析裸 variant 名不解析路径头 `a`。**逃生路径**：① 在类型标注上下文用裸 variant 名〔如 `let x: Color = Red(args)`、type-directed 解析 ③〕；② 改名消歧〔如 `fn make_color` / `enum Color`、与 §8 类别消歧 fn+constraint 同名「须改名消歧」处理范式对称〕。**soundness 声明收窄**：类型域同名绑定仅在**类型位 bare ident 上下文**〔如 `let x: Color`〕可达、不被值遮蔽；值位点分路径引用类型域 member（variant 构造）在 value+enum/error 同名下不可达、须上述逃生路径。此为病态罕见场景（enum/error+value 同名），值优先最大化点分路径可预测性、不值得为它引入回退 tie-break 或新语法；
...
| §4 loop 为语句（非 value 表达式，break 无值）——block 尾定型见 §6 rule 6（break 可达性） | §15.1 `loop` 表达式类型 = `never`（⊥）为**表达式形态**表征（类型层、非句法层声明）+ §27 loop 冻结于 `stmt` 产生式（从未入 `expr`、§4 维持此现状、非演进） | ✅ 一致（loop 未入 expr 产生式，非演进；block 尾 never / void 由 §6 rule 6 判定，非机械搬运 §15.1） |
...
| §6 rule 6 loop block 尾定型 = break 可达性（无可达 `break` → `never`；含可达 `break` → `void`） | §15.1 `loop` 表达式类型 = `never`（表达式形态表征、类型层）+ §16 loop-break 惯用法 `loop { work(); if done { break } }` + §27 loop 冻结于 `stmt`（从未入 `expr`、§4 维持此现状、非演进） | ✅ soundness 闭合（修正「无条件 `loop` → never」会接受缺返回值函数 `fn f() -> int { loop { if done { break } } }` 的缺口；对齐 Rust `loop { break; }` 类型 = `()` 而 `loop {}` = `!`；可达 break 判定为保守语法分析、可判定） |
...
| §8 step1 点分路径 value+type 同名 tie-break（值优先；value+enum/error 同名下 `EnumType.variant` 点分路径不可达、须改名或类型位裸名） | §27 `a.b` dotted-path + §44 line 2044 `Enum.Variant` 标准形 + §9 line 379 无 `Self` 关键字 + §7 turbofish 仅泛型可调用实体 + §8 type-directed ③ 裸名 | ✅ 诚实登记（与本表 turbofish 非逃生口行「fn+constraint 须改名消歧」范式对称、但冲突空间不同：turbofish 行 / `Name(args)` tie-break 类型侧仅 constraint call 形、本处类型侧有 variant member；逃生=类型位裸名 type-directed〔§8 ③〕/ 改名消歧） |
...
**收敛轨迹**：已跑多轮对抗式 workflow（pass-2 → pass-14，与 RFC 0005/0007 同批 pipeline，每轮修正后 FRESH 重跑、无 `resumeFromRunId`）。历轮累计修正：pass-2 报 1H/5M/5L（11 条，含 §3 Conformance 重定向后的回归扫描、`<` 消歧方案 B 迁移清单、EBNF 产生式自洽、decidability 论证）；pass-6/pass-10/pass-11 多轮 M/L（类型导向 variant 解析、`Err` 无包装、actor message 解析等）；**pass-14 报 2L 并已全部修正**：① §6 rule 6 block 尾定型示例 match arm 误用胖箭头 `=>` → 改冻结 §16/§27 锁定的 `Pattern: { body }` 形；② §8/§9 `WebSearch.query` 引用由「§23 / §60」收窄至**仅 §60**（§23.1/§40.2 的 `web.search` 形与自身 `tool Search` 声明不一致、为本 RFC 点分路径规则不覆盖的冻结残留示例）。post-pass-14 待 pass-15 复验收敛。**pass-15 报 1L 并已修正**（0005/0007 本 pass clean、0 confirmed）：§6 rule 4 仅枚举 §5 `literal` 内**标量**字面量（`int_lit`/`float_lit`/`char_lit`/`string_lit`/`true`/`false`），遗漏 §27 `primary`（line 1535）另两个终结符 `list_lit`/`map_lit`——空容器 `let x = []` 元素类型 `T` 无源（无元素可 synth）、既非唯一类型亦非已报错误，使 §6 decidability 声明「总是产生唯一类型或报告类型错误」对该边界 case 悬空。修正=§6 rule 4 增补容器字面量 synth/check 子规则：① 非空 `[e₁,…,eₙ]` synth 各元素并合一（不一致报错）、容器种类默认 `List<T>`（§15.9 line 604）；② 非空 `[k₁:v₁,…]` synth 键/值分别合一；③ **空容器** check 上下文从期望类型取元素类型（`check([], List<int>)`→`T=int`），synth 上下文无期望（`let x = []`）**报告类型错误**「需类型标注确定元素类型」（对齐 Rust「type annotations needed」），闭合 decidability 悬空。post-pass-15 待 pass-16 复验。**pass-16 报 2M 并已全部修正**（0005 本 pass clean、0 confirmed）：① §6 rule ③ 空 `[]` check `Array<T,N>` 缺 `N=0` 约束——pass-15 补容器字面量规则时 rule ① 非空 `Array<T,N>` 须元素计数 = `N` 的对称条款未传到空容器分支，`check([], Array<int, 5>)` 会以空字面量匹配 N=5 内联布局、成 codegen 不初始化内存的 UB 入口；修正=rule ③ 增「空 `[]` check `Array<T,N>` 须 `N=0`、`N` 非 0 为类型错误（保守拒绝）」、与 rule ① 非空计数 = `N` 对称（计数 0 ↔ `N=0`）。② §7 方案 A 缺点引「Rust pre-2018」陷阱基线但未含 literal `type_arg` 维度——因 §27 `type_arg := type | literal`（const 值实参、§86 #3 自 v0.2.1 冻结），`x < 5 > (y)` 中整型字面量 `5` 为合法 `type_arg`（literal）、同样匹配 `type_args "(" ` lookahead 被误解析为泛型调用，歧义边界严格宽于所引 Rust pre-2018（仅覆盖类型名操作数 `f < g > (h)`）；修正=§7 方案 A 缺点补该 literal 维度论证（不改变推荐方案 B 的决断）。post-pass-16 待 pass-17 复验。**pass-17 / pass-18 报 0 confirmed**（持续收敛、无回归）。**pass-19 报 1L 并已修正**（0005/0007 本 pass 另有 findings、0006 唯一）：§6 decidability 声明括注「α ≈ 近线性」与 α 的标准定义矛盾——α(n) 为 union-find 的**反阿克曼函数**（inverse Ackermann），增长极慢、α(n) ≤ 4 对所有实际输入成立，性质是「近常数」而非「近线性」；O(n·α(n)) 的近线性恰由 α 近常数导出（若 α 真为「近线性」则 O(n·α(n)) 退化为 O(n²)、与声明的近线性意图自相矛盾）。修正=括注改「α 为反阿克曼函数（近常数、α(n) ≤ 4），来自 union-find 约束求解」，使 gloss 与标准定义及 O(n·α(n)) 近线性语义一致。post-pass-19 待 pass-20 复验。**pass-20 报 0 confirmed**（本 RFC clean、持续收敛无回归）。**pass-21 报 1 confirmed [0H/0M/1L] 并已修正**（name-resolution-alignment）：§8 点分路径 `a.b` 解析步骤 1「`a` 按简单名解析（值或类型或模块）」缺与下文 `Name(args)`（line281）「值命名空间优先」对称的 tie-break——当 `a` 在同一作用域层同时命中值与类型命名空间时（合法情形：`struct Foo`/`type Foo` 与 `fn Foo`/`let Foo` 同名、命名空间分离、声明层无遮蔽），解析步骤无决断、致引用层 `a.b` 取值域还是类型域绑定未定义。修正=步骤 1 补 tie-break「值命名空间优先」（与 line281 Name(args) 对称——点分路径为值位表达式、取值域绑定；类型域同名绑定在类型位上下文仍可达、不被值遮蔽，此 tie-break 仅决断值位点分路径的引用层选择、不改变声明层分离）。post-pass-21 待 pass-22 复验收敛。**pass-22 / pass-23 复验 0006 clean**（pass-22 findings 落 0005、pass-23 finding 落 0007 §4 line 80 求值序位置子表达式枚举、0006 两轮均 0 confirmed）。**pass-24 报 1 confirmed [0H/1M/0L] 并已修正**（name-resolution-alignment，pass-21 tie-break 的次级后果）：pass-21 为步骤 1 补「值命名空间优先」tie-break 闭合了「`a` 同时命中值/类型命名空间时无决断」的直接缺口，但该 tie-break 有未登记的次级后果——当 `a` 同时为 enum/error 类型与值域绑定（合法同名、如 `enum Color { Red }` + `const Color = 5`）时，值位点分路径 `a.b`（如 `Color.Red(args)`——line 278 标准 variant 构造形 / 冻结 §44 line 2044 `Enum.Variant`）经 step 1 路由到值域绑定后、step 2 line 266 在该值上查 `b` member（基础类型无 variant 字段）→ `UnresolvedMember`，类型域解释（`a` 为 enum、`.b` 为 variant）已在 step 1 被阻断、不可达；且无点分路径逃生口（无 `Self` 关键字〔冻结 §9 line 379 仅小写 `self`〕、turbofish `::<T>`〔§7 方案 B〕仅附加于泛型可调用实体而 enum 非 generic、type-directed 解析〔第四机制 line 247-253〕仅解析裸 variant 名不解析路径头 `a`）；原 tie-break 的「类型域同名绑定在类型位上下文仍可达、不被值遮蔽」soundness 声明过宽（仅 bare type ident 可达如 `let x: Color`、dotted variant 构造不可达），「与 line 281 Name(args) tie-break 对称」claim 结构不精（二者冲突空间不同：line 281 类型侧 call 形仅 constraint `T(expr)`〔line 276〕+ Builtin 位置构造器〔line 277〕、enum 类型无 `EnumType(args)` 形、为 fn vs constraint 二者皆具 `(args)` 形的真歧义；本处类型侧有有效 dotted member 即 variant〔line 267〕、为 value-field vs type-variant，值优先阻断 variant 构造）。修正=步骤 1 tie-break：① 补后果登记（value+enum/error 同名下 `EnumType.variant` 点分路径不可达）；② 补逃生路径（类型标注上下文用裸 variant 名 `let x: Color = Red(args)` 走 type-directed〔line 251 ③〕、或改名消歧 `fn make_color`/`enum Color` 与 line 281 fn+constraint 处理范式对称）；③ soundness 声明收窄为「类型位 bare ident 上下文可达」；④「与 line 281 对称」改辨析（冲突空间不同、仅改名消歧处理范式对称）；⑤ §9 自洽核查表补登记行（与 line 309 turbofish 非逃生口的「诚实登记」范式对称）。post-pass-24 待 pass-25 复验收敛。**pass-25 报 2 confirmed [0H/0M/2L] 并已全部修正**：① [L] lex-priority——§5 承诺模型已显式覆盖三类 `_` 边界违例（前导零 `0123`/`0_5`、进制前缀+越界数字 `0b2`、裸尾 `_` `123_`）、但连续 `_`（`1__0`/`0xFF__1`/`1.0__5`/`1e0__5`）未入任何条款；连续 `_` 中第一个 `_` 已被词法尝试消费为分隔符却未后随数字（紧接第二个 `_` 违约）、与裸尾 `_` 同构，承诺模型覆盖边界（前导零+裸尾 `_`）与其自身依据（`_` 作分隔符须后随数字）之间存在论证缺口、未补时 `1__0` 经 maximal munch 静默拆为 `int 1` + `ident __0`（`__0` 匹配 §8.6 标识符允许 `_` 起首）延后报 parser 困惑错（恰承诺模型设计来消除的那类）→ §5 补「连续 `_` 承诺」条款：数字字面量内 `_` 后须紧跟数字、否则 AIL1xxx 词法错、不回退为「字面量+ident」拆分；三边闭合（前导侧前导零承诺/尾部侧尾部 `_` 承诺/内部侧本条）；注本 RFC `decimal := [1-9] ("_"? [0-9])*` 与 Python `\d(_?\d)*` 同构、Python 亦拒绝 `1__0`（`SyntaxError`）、本条款使 AILang 词法层即清晰拒绝。② [L] semantic-no-conflict——RFC 对 loop 是否曾在 expr 产生式中给出自相矛盾论证：§4 line97「loop 未加入 §27 expr 产生式…非演进」+ §11#1 line331「loop 未入 expr 产生式」+ §9 line294 自洽性列「loop 未入 expr 产生式，非演进」均正确（冻结 §27 line1515 loop 在 stmt、expr 产生式链 line1520-1535 不含 loop、loop 自 v0.2.1 起仅在 stmt 从未入 expr），但 §6 rule6 line173 两处「§4 将 loop 降为语句（移出 expr）」「§4 既已将 loop 移出 expr 降为语句」+ §9 line294/300 既有规范列「§4 loop 移出 expr 降为语句」——「移出 expr」蕴含 loop 先前在 expr、与上述四处矛盾、§9 line294 同行既有规范列（移出 expr=演进）与自洽性列（非演进）自相矛盾却标 ✅ 一致；「移出 expr」措辞字面采纳还暗示 RFC 从冻结 §27 expr 删除 loop（=修改冻结规范、与 §1「不触动冻结、唯一例外 §7」冲突）→ 改四〔§6 line173 + §9 line294 + §9 line300〕处「移出 expr 降为语句」为「冻结 §27 loop 仅在 stmt 产生式、从未入 expr、§4 维持此现状、非演进」、§6 line173 同时点明 §15.1「loop 表达式类型为 never」为类型层表达式形态表征（非句法层声明），与 §4/§11#1/§9 自洽性列措辞统一、消除同行矛盾。post-pass-25 待 pass-26 复验收敛。审查维度：EBNF 产生式自洽 / 词法优先级无冲突 / 推断算法 decidability 证明严谨性 / `<` 消歧两方案 tradeoff 完备性 / 名字解析与 §89 §92 §12.2 对齐 / 与 §16 §87 §86 既有语义无矛盾 / 关键字地位 / AI 可消费性。
