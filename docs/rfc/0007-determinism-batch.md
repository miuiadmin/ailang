...
| §8 length 字节 | §36 length 返回 int | ✅ |
| §8 char 位复制 Copy | §18.4 三分类（追加 char，同 byte）+ RFC 0006 §5 char_lit | ✅ |
