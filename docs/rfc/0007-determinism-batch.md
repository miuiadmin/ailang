...
**与 RFC 0005 的交叉**：求值序与 Map 迭代序当前在 RFC 0005 §6 登记为「未指定行为」；本 RFC 将其**收紧为定义行为**（求值序）或**显式锁定为未指定**（Map 迭代，诚实登记），二者都更新 RFC 0005 §6 的状态（§10）。
...
4. **混合序**：同一作用域内，locals 与临时值统一按**析构时点逆序** drop（后构造 / 后声明的先释放）。
...
> 此决断把 RFC 0005 §6 + 附录 B 登记的「Map 迭代序」从「未提及」**显式锁定为未指定**（诚实登记，非假装有序）。
